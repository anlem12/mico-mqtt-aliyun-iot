...
/**
  ...
  */
static IoT_Error_t _mqtt_serialize_unsubscribe(unsigned char *pTxBuf, size_t txBufLen,
	...
	for(i = 0; i < count; ++i) {
		mqtt_internal_write_utf8_string(&ptr, pTopicNameList[i], pTopicNameLenList[i]);
	}
...
/**
  ...
  * @return IoT_Error_t indicating function execution status
  */
static IoT_Error_t _mqtt_deserialize_unsuback(uint16_t *pPacketId, unsigned char *pRxBuf, size_t rxBufLen) {
