...
/**
  ...
  * @return IoT_Error_t indicating function execution status
  */
static IoT_Error_t _mqtt_serialize_connect(unsigned char *pTxBuf, size_t txBufLen,
	...
	ptr = pTxBuf;
	len = _mqtt_get_connect_packet_length(pConnectParams);
	if(mqtt_internal_get_final_packet_length_from_remaining_length(len) > txBufLen) {
	...
	mqtt_internal_write_char(&ptr, flags.all);
	mqtt_internal_write_uint_16(&ptr, pConnectParams->keepAliveIntervalInSec);
	...
	if(NULL == pConnectParams->pClientID) {
		mqtt_internal_write_uint_16(&ptr, 0);
	} else {
		mqtt_internal_write_utf8_string(&ptr, pConnectParams->pClientID, pConnectParams->clientIDLen);
	}

	if(pConnectParams->isWillMsgPresent) {
		mqtt_internal_write_utf8_string(&ptr, pConnectParams->will.pTopicName,
												pConnectParams->will.topicNameLen);
		mqtt_internal_write_utf8_string(&ptr, pConnectParams->will.pMessage, pConnectParams->will.msgLen);
	}

	if(flags.bits.username) {
		mqtt_internal_write_utf8_string(&ptr, pConnectParams->pUsername, pConnectParams->usernameLen);
	}

	if(flags.bits.password) {
		mqtt_internal_write_utf8_string(&ptr, pConnectParams->pPassword, pConnectParams->passwordLen);
	}
...
/**
  ...
  * @return IoT_Error_t indicating function execution status
  */
static IoT_Error_t _mqtt_deserialize_connack(unsigned char *pSessionPresent, IoT_Error_t *pConnackRc,
	...
	flags.all = mqtt_internal_read_char(&curdata);
	*pSessionPresent = flags.bits.sessionpresent;
	connack_rc_char = mqtt_internal_read_char(&curdata);
 ...
 */
static bool _mqtt_is_client_state_valid_for_connect(ClientState clientState) {
	bool isValid = false;
...
IoT_Error_t mqtt_connect(MQTT_Client *pClient, IoT_Client_Connect_Params *pConnectParams) {
	...
	if(false == _mqtt_is_client_state_valid_for_connect(clientState)) {
	...
	mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTING);
	...
	if(MQTT_SUCCESS != rc) {
		...
		mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTING, CLIENT_STATE_CONNECTED_IDLE);
	}
...
IoT_Error_t mqtt_attempt_reconnect(MQTT_Client *pClient) {
	...
	rc = mqtt_connect(pClient, NULL);

	/* If still disconnected handle disconnect */
	if(CLIENT_STATE_CONNECTED_IDLE != mqtt_get_client_state(pClient)) {
		mqtt_set_client_state(pClient, CLIENT_STATE_DISCONNECTED_ERROR, CLIENT_STATE_PENDING_RECONNECT);
